...
public class UsuarioService implements UserDetailsService {
    ...
    public Usuario registrarUsuario(Usuario usuario, String passwordSinEncriptar) {
        ...
        usuario.setFechaEnvioVerificacion(LocalDateTime.now());
        usuario.setFechaRegistro(LocalDateTime.now());  // ✅ AGREGADO: Fecha de registro
    ...
    public boolean verificarCorreo(String token) {
        ...
        if (usuarioOpt.isPresent()) {
            ...
            if (usuario.getFechaEnvioVerificacion() != null) {
                long horas = java.time.Duration.between(usuario.getFechaEnvioVerificacion(), LocalDateTime.now()).toHours();
    ...
    public void crearAdminSiNoExiste() {
        ...
        if (adminExistente == null) {
            ...
            admin.setSeccionesAcceso(null);  // ✅ AGREGADO: Secciones de acceso para ADMIN
            admin.setFechaRegistro(LocalDateTime.now());  // ✅ AGREGADO
            usuarioRepository.save(admin);
            System.out.println("✅ Usuario ADMIN creado: admin / 123456");
        }
        
        if (usuarioRepository.countByRol("USUARIO") == 0) {
            ...
            user.setFechaRegistro(LocalDateTime.now());  // ✅ AGREGADO
    ...
    public boolean esCorreoValido(String email) {
        ...
        if (dominio.contains("omniview.com")) {
            return false;
        }
        if (!dominio.contains(".")) {
            return false;
